...
import { isQuinielaArchived, type RoundSelected } from "@/lib/rounds";
import ArchivedQuinielasSection from "@/components/shared/ArchivedQuinielasSection";

interface QuinielaCardData {
  id: string;
  name: string;
...
function QuinielaCard({
  ...
  quiniela: QuinielaCardData;
  isOwner: boolean;
  archived?: boolean;
}) {
  return (
    <Link href={`/quinielas/${quiniela.id}`} className="group">
      <Card
        className={`relative h-full overflow-hidden border-border/50 transition-all duration-300 hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5 ${
      ...
      >
        ...
        <CardContent className="relative p-4 sm:p-5">
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            {archived && (
              <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground sm:gap-1.5 sm:px-2.5 sm:py-1 sm:text-xs">
                Finalizada
              </span>
            )}
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground sm:gap-1.5 sm:px-2.5 sm:py-1 sm:text-xs">
              ...
              {new Date(quiniela.joinedAt).toLocaleDateString("es-ES", {
                day: "numeric",
                month: "short",
              })}
            </span>
...
export default async function QuinielasPage() {
  ...
  const activeQuinielas = userQuinielas.filter(
    (q) => !isQuinielaArchived(q.roundsSelected ?? []),
  );
  const archivedQuinielas = userQuinielas.filter((q) =>
    ...
    <div className="max-w-6xl px-4 py-6 sm:ml-6 sm:mt-6">
      ...
      {userQuinielas.length === 0 ? (
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {activeQuinielas.map((quiniela) => (
              <QuinielaCard
                key={quiniela.id}
                quiniela={quiniela}
                isOwner={quiniela.ownerId === session.user.id}
              />
            ))}
          ...
          {/* Archived (finished > 1 month ago) Quinielas */}
